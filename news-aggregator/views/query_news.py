...
# Update this to point to your analysis folder
...
def find_analysis_file(date_key):
...
def show():
    st.header("Search News by Natural Language Date")
    user_input = st.text_input("Enter a date (e.g., 'May 21', '21 May 2025'): ")

    if user_input:
        date_key = get_date_key(user_input)
        if not date_key:
            st.error(f"❌ Couldn't parse the date from input: {user_input}")
            return

        file_path = find_analysis_file(date_key)

        if file_path:
            st.success(f"📰 Showing analyzed news from: {os.path.basename(file_path)}")
            st.markdown(load_file_content(file_path))
        else:
            st.error(f"❌ No analyzed news found for {user_input} (looking for analysis_{date_key}_*.md).")
...
def find_analysis_files(date_key):
    pattern = os.path.join(ANALYSIS_DIR, f"analysis_{date_key}_*.md")
    matches = glob.glob(pattern)
    matches.sort()  # Optional: sort by time if filenames contain it
    print(f"Looking for files with pattern: {pattern}")
    print(f"Matches found: {matches}")
    return matches
...
def show():
    ...
    if user_input:
        ...
        file_paths = find_analysis_files(date_key)

        if file_paths:
            st.success(f"📰 Found {len(file_paths)} news files for: {user_input}")
...
def show():
    ...
    if user_input:
        ...
        matches = find_analysis_files(date_key)

        if matches:
            st.success(f"📰 Found {len(matches)} analyzed news file(s) for: {user_input}")
